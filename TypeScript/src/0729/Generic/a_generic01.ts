...
//! 제네릭 장점
// 1. 타입 안정성 보장: 컴파일 시점에서 타입을체크, 런타임 시점에서 발생할 에러를 방지
// 2. 재사용 가능한 컴포넌트를 생성
// 3. 타입 추론 개선: 
...
//! 제네릭의 타입 변수
// 타입변수: 입력인수(매개 변수)의 타입을 T로 지정하고, 함수의 반환 타입을 T로 지정한다

//? 단일 타입 변수
// 위의 generic 함수 구현이 단일 타입 변수

//? 여러 타입 변수
// 함수가 여러 개의 독립적인 타입을 처리해야할때 사용

function pair<T, U>(frist: T, second: U): [T, U] {
  return [frist, second]
}

let output3 = pair<string, number>("myString", 100);
// let output3 = pair<string, number>(100, "myString"); // 순서를 지켜서 적어야함 이렇게하면오류남
