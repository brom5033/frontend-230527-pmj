...
class InformationEvent {
  ...
  addEventPhotoChangeClick(): void {
    const inforPhoto = document.querySelector<HTMLElement>('.info-photo');
    if (inforPhoto) {
      inforPhoto.onclick = () => {
  ...
  addEventAboutMeModifyClick(): void {
    ...
    if (aboutMeModifyButton) {
      aboutMeModifyButton.onclick = () => {
        ...
        infoInputContainers.forEach((infoInputContainers) => {
          // 각 컨테이너 안의 info-input 요소를 찾아서 변수에 저장
          const infoInput = infoInputContainers.querySelector<HTMLInputElement>('.info-input');
          if(infoInput) {
  ...
  addEventIntroduceModifyClick(): void {
    const introduceModifyButton = document.querySelector<HTMLButtonElement>('.s-introduce');
...
addEventIntroduceSaveClick(): void {
  ...
  if (introduceSaveButton) {
    introduceSaveButton.onclick = () => {
      const introduceModifyButton?.classList.remove('button-hidden')
      const introduceModifyButton.classList.add('button-hidden')

      const introduceInput = document.querySelector<HTMLInputElement>(',introduce-input');
      if (introduceInput) {
        introduceInput.disabled = true;
        const userInfo = IntormationService.getInstance().userInfo;
