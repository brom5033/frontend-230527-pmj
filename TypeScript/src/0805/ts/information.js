// 사용자 정보를 수정 하고 저장하는 이벤트를 관리하는 클래스
var InformationEvent = /** @class */ (function () {
    ...
    InformationEvent.getInstance = function () {
        if (this.instance == null) {
            // (인스턴스가 없을때만) 인스턴스 새로 생성
            this.instance = new InformationEvent();
        }
        return this.instance;
    };
    // 사용자의 프로필 사진 클릭시 사진 변경을 위한 파일 입력창을 여는 이벤트
    InformationEvent.prototype.addEventPhotoChangeClick = function () {
        var inforPhoto = document.querySelector('.info-photo');
        if (inforPhoto) {
            inforPhoto.onclick = function () {
    ...
    InformationEvent.prototype.addEventAboutMeModifyClick = function () {
        ...
        if (aboutMeModifyButton) {
            aboutMeModifyButton.onclick = function () {
                ...
                infoInputContainers.forEach(function (infoInputContainers) {
                    // 각 컨테이너 안의 info-input 요소를 찾아서 변수에 저장
                    var infoInput = infoInputContainers.querySelector('.info-input');
                    if (infoInput) {
    ...
    InformationEvent.prototype.addEventIntroduceModifyClick = function () {
        var introduceModifyButton = document.querySelector('.s-introduce');
        introduceModifyButton === null || introduceModifyButton === void 0 ? void 0 : introduceModifyButton.classList.remove('button-hidden');
        introduceModifyButton === null || introduceModifyButton === void 0 ? void 0 : introduceModifyButton.classList.add('button-hidden');
...
void {
    ...
    if: function (introduceSaveButton) {
        introduceSaveButton.onclick = function () {
            var introduceModifyButton, classList;
        };
    },
    : .remove('button-hidden'),
    const: introduceModifyButton.classList.add('button-hidden'),
    const: introduceInput = document.querySelector(',introduce-input'),
    if: function (introduceInput) {
        introduceInput.disabled = true;
        var userInfo = IntormationService.getInstance().userInfo;
...
var FileService = /** @class */ (function () {
    ...
    // : 사이즈가 0이 아닌경우에만 showPreview(미리보기) 함수를 호출
    ...
    FileService.prototype.showPreview = function (fileValue) {
        ...
        // 파일이 성공적으로 읽혀졌을 때 실행되는 콜백함수
        fileReader.onload = function (e) {
            var _a;
            // 읽은 결과물이 e.target?.value에 있을 경우 변수에 할당
