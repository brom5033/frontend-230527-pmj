// API: 웹브라우저가 제공해주는 기능
// 웹캠사용, 사용자에게 알림을 제공, 사용자의 위치를 추적 등
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 값을 읽을때는 getItem()사용
  // localStorge.input도 가능

  if(savedValue) {
    input.value = savedValuep
    p.textContent = '이전 실행한 마지막 값: ${savedValue}'
  }

  input.addEventListener('keyup', (event) => {
    const value = event.currentTarget.value
    localStorage.setItem('input', value)
    // 값을 저장할때는 setItem()메소드를 사용
    // localStorge.input도 가능
...
// 객체나 배열을 불러올땐 JSON.parse()를 사용해 원래 형태로 복원